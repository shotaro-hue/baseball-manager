...
以下 3 つの spec ファイルと GitHub Actions ワークフローを新規追加する（既存の `e2e/title.spec.js` は変更しない）。
...
4. **`.github/workflows/e2e.yml`** — PR・push 時に GitHub Actions 上で Playwright を自動実行する CI ワークフロー
...
| `.github/workflows/e2e.yml` | Create | GitHub Actions CI ワークフロー |
...
### Step 4: .github/workflows/e2e.yml（GitHub Actions CI）

PR・push 時に Playwright テストを自動実行するワークフロー。
`ubuntu-latest` ランナーは Chromium の依存ライブラリを含むため、`--with-deps` で確実にインストールできる。
`playwright.config.js` の `webServer` 設定（`npm run dev`）がそのまま使われるため、追加設定は不要。
...
**ポイント:**
- `branches` に `'claude/**'` を含めることで、Codex が作業するブランチでも自動実行される
- テスト失敗時は `playwright-report/` を artifact としてアップロードし、GitHub UI からダウンロードして確認できる
- `timeout-minutes: 15` でハングアップを防止
- `playwright.config.js` の `webServer.reuseExistingServer: !process.env.CI` が既に設定済みのため、CI では必ず新規 dev server を起動する
...
- [ ] 各 spec ファイルが `import { test, expect } from '@playwright/test'` を正しく使用している
- [ ] `npm run build` でビルドエラーなし（本体コードを変更しないため自明）
- [ ] （GitHub Actions / ローカル）`npx playwright install --with-deps chromium && npx playwright test` で全テストがパスする
...
- .github/workflows/e2e.yml: PR・push 時に GitHub Actions で Playwright を自動実行する CI を追加
- T7-E2E を ✅ 完了に更新
```
