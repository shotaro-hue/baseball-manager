...
function generateSyntheticSprayPoints(leftRatio, centerRatio, rightRatio, sampleSize) {
  const safeLeft = Math.min(1, Math.max(0, Number.isFinite(Number(leftRatio)) ? Number(leftRatio) : 0));
  const safeCenter = Math.min(1, Math.max(0, Number.isFinite(Number(centerRatio)) ? Number(centerRatio) : 0));
  const safeRight = Math.min(1, Math.max(0, Number.isFinite(Number(rightRatio)) ? Number(rightRatio) : 0));
  const totalRatio = safeLeft + safeCenter + safeRight;
  if (totalRatio <= 0) return [];

  const totalPoints = Math.min(120, Math.max(12, Number.isFinite(Number(sampleSize)) ? Math.round(sampleSize) : 36));
  const normalized = {
    left: safeLeft / totalRatio,
    center: safeCenter / totalRatio,
    right: safeRight / totalRatio,
  };
  const leftCount = Math.round(totalPoints * normalized.left);
  const centerCount = Math.round(totalPoints * normalized.center);
  const rightCount = Math.max(0, totalPoints - leftCount - centerCount);

  const buildPoint = (segment, index, count) => {
    const t = (index + 1) / (count + 1);
    const angleBand = segment === 'left' ? [2, 28] : segment === 'right' ? [62, 88] : [32, 58];
    const distanceBand = [25, 205];
    const angle = angleBand[0] + (angleBand[1] - angleBand[0]) * t;
    const dist = distanceBand[0] + (distanceBand[1] - distanceBand[0]) * ((index * 37) % 100) / 100;
    return { sprayAngle: angle, dist, result: 'out' };
  };

  return [
    ...Array.from({ length: leftCount }, (_, i) => buildPoint('left', i, leftCount)),
    ...Array.from({ length: centerCount }, (_, i) => buildPoint('center', i, centerCount)),
    ...Array.from({ length: rightCount }, (_, i) => buildPoint('right', i, rightCount)),
  ];
}

function SprayDistributionChart({ sprayPoints, leftRatio, centerRatio, rightRatio, sampleSize }) {
  const safePoints = Array.isArray(sprayPoints)
    ? sprayPoints.map(sanitizeSprayPoint).filter(Boolean)
    : [];
  const syntheticPoints = safePoints.length > 0
    ? []
    : generateSyntheticSprayPoints(leftRatio, centerRatio, rightRatio, sampleSize).map(sanitizeSprayPoint).filter(Boolean);
  const pointsForPlot = safePoints.length > 0 ? safePoints : syntheticPoints;
  const hasData = pointsForPlot.length > 0;
  const plottedPoints = pointsForPlot.map((point, idx) => {
