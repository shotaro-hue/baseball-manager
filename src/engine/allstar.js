 ...
 * オールスター2試合を実行して返す
 */
export function runAllStarGames(rosters, year, gameDates = []) {
  const venues = getAllStarVenues(year);
  const game1 = { ...simulateAllStarGame(rosters, true), gameNo: 1, venue: venues[0], date: gameDates[0] || null };
  const game2 = { ...simulateAllStarGame(rosters, false), gameNo: 2, venue: venues[1], date: gameDates[1] || null };
  return [game1, game2];
}
