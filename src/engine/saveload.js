...
function mergeCareerLogSummary(base, delta) {
  ...
  const mergedLastYear = [baseLastYear, deltaLastYear]
    .filter((year) => year > 0)
  ...
  return {
    ...
    lastYear: Number.isFinite(mergedLastYear) ? mergedLastYear : 0,
...
function logTeamsBreakdownSize(state) {
  ...
  state.teams.forEach((team) => {
    ...
    topPlayers.forEach(({ player, byteSize }, index) => {
      ...
      const careerLogSize = getJsonByteSize(player.careerLog);
      const careerLogRatio = byteSize > 0 ? ((Math.max(careerLogSize, 0) / byteSize) * 100).toFixed(1) : '0.0';
      console.info(`[SaveSize:player.careerLog] ${teamName} name=${playerName} careerLogSize=${formatJsonSize(careerLogSize)} ratio=${careerLogRatio}%`);
