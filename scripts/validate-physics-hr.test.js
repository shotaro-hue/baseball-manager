    ...
    () => {
      ...
      const results = profiles.map((profile) => {
        ...
        for (let i = 0; i < TOTAL_PA; i += 1) {
          ...
          const q = ['weak', 'normal', 'solid', 'hard', 'barrel'].includes(meta.quality)
            ? meta.quality
            : 'normal';
        ...
        return {
          ...
          qualityRate: Object.fromEntries(
            Object.entries(counter.quality).map(([k, v]) => [k, v / Math.max(counter.bip, 1)])
          ),
        };
      });
