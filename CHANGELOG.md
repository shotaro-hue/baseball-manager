...
### 2026-05-03 — fix: バッチシム完了後に画面が次へ進まない問題を修正

**仕様本文への影響なし（内部バグ修正のみ）**

- 根本原因: `setAllTeamResultsMap` をチャンク方式（30件×16回）で更新していたため、チャンクごとに React が再レンダリングを行い、モバイルで著しく処理が遅くなっていた。`setBatchProgress(null)` と `setScreen("batch_result")` が呼ばれるまでに時間がかかりすぎ、プログレスバーが 65/65 のまま止まって見えた。
- 修正: チャンク分割ループを廃止し、全試合分の差分を先に `patches` オブジェクトにまとめてから `setAllTeamResultsMap` を1回だけ呼ぶ O(n) 方式に変更。書き込まれるデータ（打者・投手成績・スコア）は変更なし。

---

### 2026-05-02 — fix: 異常打高（打率0.425）の根本修正

**仕様本文への影響なし（内部バグ修正のみ）**

- 根本原因①: `LA_NOISE=12` / `LA_DEFAULT_MID=12` で打球角度が0°〜24°に集中、ライナー67%・フライ0%の非現実的分布（NPB実績: ゴロ45%/ライナー20%/フライ35%）
- 根本原因②: 「安全弁」コードがアウトの20%を単打に変換し、逆方向は5.5%のみで安打を大量に上積みしていた
- 根本原因③: フライ捕球率が0.66と低く、フライ発生時のBABIPも高止まりしていた
- 修正: `LA_NOISE 12→16` / `LA_DEFAULT_MID 12→8`、フライ判定閾値 25°→18°、フライ捕球率 0.66→0.88（浅）/ 0.50→0.78（中）/ 0.34→0.55（深）、ライナー捕球率 0.44→0.55 / 0.34→0.45、安全弁アウト→単打変換を削除
- 期待効果: BABIP 0.55→0.29、打率 0.425→約0.244（NPB基準相当）
...
- `src/components/Baseball3DModal.jsx` の打球速度チップ表示を `mph` から `km/h` に変更
- 画面内の表示文言を検索（`mph`, `打球速度`）し、UI上で `km/h` 以外の打球速度単位が残っていないことを確認
- `src/components/ui.jsx` の用語説明（「打球の初速（km/h）」）と整合していることを再確認
- ⚠️ 既存セーブデータへの影響なし: 内部保存値の変換・マイグレーション【＝保存形式の変更処理】は実施していない
...
- 根本原因①: `EV_FLOOR=112mph` が非現実的に高く、弱い打球でも MLB 最強打者レベルの EV が生成されていた（NPB 平均 ~88mph に対し最低値が 112mph）
- 修正①: `src/constants.js` の `EV_FLOOR: 112 → 72`、`EV_POWER_SCALE: 34 → 33`、`EV_STUFF_SCALE: 16 → 14` に変更。power=50 打者の平均 EV が ~129mph → ~89mph に正常化
- 根本原因②: `adjustResultByPhysics` が `'hr'` と `'d'` のみを補正対象としており、シングル・トリプルは飛距離チェックを受けずログに「dist=150m, result=s」が残っていた
- 修正②: `src/engine/simulation.js` の `adjustResultByPhysics` を `'s'` と `'t'` にも拡張。フェンス距離+8m を超えた打球はすべて HR に変換

---

### 2026-04-30 — fix: 3D打球リプレイのスプレー角反転・軌跡距離ズレを修正、視認性を改善

**仕様本文への影響なし（内部バグ修正のみ）**

- 根本原因①（B1）: `BallTrajectory` のアジマス計算が `sprayAngle * PI/180` のため、小角度（左翼判定）が RF 方向へ飛んでいた
- 修正①: アジマスを `(sprayAngle - 45) * PI/180` に変更し `x=sin(az)*dist`、`z=cos(az)*dist` で LF/RF を正しく一致させた
- 根本原因②（B2）: モーダルが `DRAG_COEFF=0.18` の独自近似を使用しており、表示飛距離（`calcBallDist` 結果）と 3D 着地点がズレていた
- 修正②: `physics.js` に `calcTrajectory(ev, la)` を追加（同一 `DRAG_COEFF=0.35` で数値積分）し、モーダルのアニメーションをこの軌跡で統一
- 視認性改善: ベースダイヤモンド・投手マウンド・ファウルポール・フェンス距離ラベルを追加、カメラ低角度化・ズーム有効化・ボール影を追加

---

### 2026-04-29 — feat: CPU球団フロントAIを戦略モード対応に拡張

**仕様本文への影響あり（トレードAI挙動）**

- `frontOfficePlan`（`contend/retool/rebuild/neutral`）を評価する `evaluateFrontOfficePlan` を追加
- トレード評価を「現在価値・将来価値・契約価値」の重み付き計算へ変更し、モード別で優先度を切替
- `rebuild` 時の若手重視補正、`contend` 時の即戦力重視補正を `evalTradeForCpu` に追加
- シーズン進行中（CPUオファー生成・CPU同士デッドライントレード）にフロント方針を再評価して反映
- 関連ユニットテスト（戦略判定/モード別評価）を追加

---

### 2026-04-29 — docs: SPEC.md の最終更新日と変更履歴リンクを更新

**仕様本文への影響あり（ドキュメント整備のみ）**

- `SPEC.md` の先頭「最終更新」表記を 2026-04-29 に更新
- 目次の「変更履歴」リンクを `../CHANGELOG.md` から `./CHANGELOG.md` に修正（リポジトリ直下配置と整合）

---

### 2026-04-26 — balance: 打率過剰・盗塁数過剰を NPB 実績に近づける調整

**仕様本文への影響あり（§7 ゲームバランス定数）**

- `contact.hi`: 0.260 → 0.240（上位層打率の上限を旧値に戻し、首位打者を .370→.330 前後に）
- `hitMod`: 1.10 → 1.02（全体打率の底上げを縮小し投高打低への逆振れを防ぎつつ引き下げ）
- 盗塁成功率ベース: 0.65 → 0.60・上限: 0.92 → 0.85（NPB 実績 70-75% 目標）
- 自動盗塁試図確率: 速80+/技70+ 28%→22%、速70+/技60+ 18%→12%、速60+/技50+ 8%→4%
...
- 根本原因: `cpuAutoManageTeam` のファーム昇格ロジックが `effScore` 順（総合スコア）で投手を昇格させるため、複数シーズン経過後に先発が引退・FA離脱しても中継ぎ系が優先昇格。結果として `newRotation` が 2 人のみになり、1 投手が 70+ 試合先発する異常が発生していた
- 修正内容: Section 1 と Section 2 の間に「先発ローテ確保ステップ（1.5）」を追加。1軍の `subtype=先発` 投手が 6 人未満の場合、farm から `subtype=先発` 選手をスコア順に優先昇格させる。空き枠がない場合は最弱の非先発投手を降格させて入れ替える（`src/hooks/useSeasonFlow.js`）

---

### 2026-04-26 — fix: 自動編成バランスレコメンドが満員時に機能しないバグを修正

**仕様本文への影響なし（内部バグ修正のみ）**

- 根本原因①: `buildRosterRecs` でロースターが28人満員（openSlots=0）の場合、昇格フェーズが `slotsLeft=0` でスキップされ、同種別スワップのみ実行 → 投手/野手バランスを崩せない構造になっていた
- 根本原因②: 同種別スワップ（`fp.isPitcher===ap.isPitcher`）は投手↔野手のバランス調整を行えない
- 修正: `buildRosterRecs` にフェーズ3（クロス種別バランス調整）を追加。投手不足時は「最弱野手 → 二軍・最強farm投手 → 一軍」のswapレコメンドを生成。野手不足時も同様。仮想ロースターを使って変更を積み上げることで重複・矛盾のないレコメンドを保証
- `cpuAutoManageTeam` にも同一クロス種別ロジックを追加

---

### 2026-04-26 — feat: 自動編成バランス改善（投手/野手比率考慮 + 全件レコメンド）

**仕様本文への影響あり（§4 ロースター管理）**

- `buildRosterRecs` に投手/野手バランスボーナスを追加: 1軍の投手数が目標（13人）を下回る場合は昇格候補の投手スコアに+20、野手数が目標（15人）を下回る場合は野手に+20を付与。降格候補のソートにも同ロジックが適用されるため、過多の種別が優先的に降格推薦される
- 昇格推薦件数を「3件固定」から「空き枠すべて」に拡張
- スワップ推薦の件数上限（6件）を撤廃し、スコア差閾値を超える全候補を列挙
- `cpuAutoManageTeam`（CPU自動編成）にも同じバランスロジックを適用
- 定数追加: `OPTIMAL_PITCHER_COUNT=13`、`ROSTER_BALANCE_BONUS=20`（`src/constants.js`）

---

### 2026-04-26 — fix: CPUローテに中継ぎ混入で先発1回交代バグを修正（B25）

**仕様本文への影響なし（内部バグ修正のみ）**

- 根本原因: `cpuAutoManageTeam` が先発5人チームのローテを6枠埋めるため `relievers.slice(0,1)` で中継ぎを補充。その中継ぎが登板すると `autoSwapPitcher` で `isStarter=false` 判定され1イニング後に強制交代
- 修正: `newRotation = starters.slice(0, 6)` に変更。先発のみをローテーションに入れ、余った中継ぎは全員ブルペンパターンへ（`useSeasonFlow.js:172`）

---

### 2026-04-26 — fix: 試合結果の投手名「?」表示 / CPU先発無条件交代バグ（B23・B24）

**仕様本文への影響なし（内部バグ修正のみ）**

- 根本原因①: `findPlayer(team, id)` が `team.players` のみ検索し `team.farm` を見ていなかった。試合後に怪我自動降格した投手が farm に移動すると名前が「?」になっていた
- 修正①: `findPlayer` に `?? team?.farm?.find(...)` フォールバックを追加（`Screens.jsx:68`）
- 根本原因②: `autoSwapPitcher` の先発交代条件 `(gs.inning >= 7 && lead > 0)` が疲弊度チェックなしで動作し、フレッシュな先発も7回リードで強制降板していた
- 修正②: 条件に `&& fatigueWarning` を追加。疲弊83%未満の先発は7回リード時も続投可能に（`simulation.js:644`）

---

### 2026-04-26 — balance: 投高打低緩和・中立化（bbMod / hrMod / hitMod 調整）

**仕様本文への影響あり（§7 リーグ環境係数）**

- `bbMod`: 1.00 → 1.30（四球率 5.6% → 目標 7.3%、NPB基準に合わせ四球を増加）
- `hrMod`: 1.00 → 1.35（HR率 1.3% → 目標 1.8%、NPB基準に合わせ本塁打を増加）
- `hitMod`: 1.05 → 1.10（打率 0.230 → 目標 0.244、安打率をさらに押し上げ）
- ERA は BB/HR/H 増加の連鎖で 2.26 → 3.5 台への改善を期待
- 変更ファイル: `src/engine/simulation.js`（DEFAULT_LEAGUE_ENV）

---

### 2026-04-26 — feat: バッチシム中自動編成オプション + リーグ全体成績ランキングタブ（U10・U11）

**仕様本文への影響なし（内部実装のみ）**

- **U10 バッチシム中自動編成オプション**: バッチシムUI（まとめてシムボタン）にチェックボックス「自動編成も実行」を追加。ONにすると7日ごとに自チームも `cpuAutoManageTeam()` で打順・ローテ・ロスターを最適化。`handleBatchSim` / `handleSeasonSim` に `autoManageMyTeam` 引数を追加。残り全試合シムボタンも同設定に連動
- **U11 リーグ全体成績ランキングタブ**: `LeaderboardTab.jsx` を新規作成し「分析」セクションに「ランキング」タブとして追加。全12球団の打者（PA≥10）・投手（IP≥5）を一覧。ヘッダークリックで昇順/降順ソート切替。現在のソート指標の上位3名を🥇🥈🥉メダルカードで強調表示。自チーム選手は青色バッジで識別
- 変更ファイル: `src/hooks/useSeasonFlow.js`・`src/App.jsx`・`src/components/tabs/LeaderboardTab.jsx`（新規）・`src/components/Tabs.jsx`
...
- 「自動編成」ボタン押下時に打順最適化と同時にロスター編成レコメンドを生成し、タブボタン下にインラインパネルとして表示
- レコメンド3種類: **昇格**（1軍枠に空きあり → 能力値上位の二軍選手を推薦）、**降格**（1軍枠超過 → 能力値下位の一軍選手を推薦）、**スワップ**（1軍枠満員でも二軍に有力選手がいる場合 → 入れ替えペアを提示）
- 各レコメンドに「▶ 実行」ボタン（個別実行）と「▶ すべて実行」ボタンを設置。実行後はパネルを自動更新
- 外国人枠（MAX_外国人_一軍=4）・登録抹消クールダウン・育成フラグ・怪我状態を考慮して候補を絞り込む
- `ROSTER_SWAP_SCORE_THRESHOLD=15` を `src/constants.js` に追加
- 変更ファイル: `src/components/tabs/RosterTab.jsx`・`src/constants.js`

---

### 2026-04-23 — balance: 財務収入カーブ調整 + fix: 契約延長オファー単位修正

**仕様本文への影響なし（内部実装のみ）**

- `calcRevenue` の計算式を見直し、予算に比例して無制限に収入が伸びる構造を廃止。予算影響は平方根ベース + 上限/下限クランプに変更し、試合ごとの収入を現実的なレンジへ調整
- 球場レベル倍率・スポンサー収入・グッズ比率を定数化し、ゲームバランス調整しやすい構成に整理
- 契約延長オファー画面の年俸入力を「万円」単位に統一修正（表示ラベルと内部値の不一致を解消）
- `finance.test.js` を追加し、収入レンジと球場投資による増収が維持されることを自動テスト化
...
- `postGame.js` の `applyGameStatsFromLog` / `computePitching` を修正
- **旧**: L（敗戦）は常に先発投手に付与（中継ぎがリードを失っても先発の L になっていた）
- **新**: ゲームログを再走査し「相手チームが最終的に勝ち越したプレイの pitcherId」を特定して L を付与（先発・救援問わず）
- 同点以降にチームが逆転した場合は losingPitcherId をリセットし、最後の勝ち越し投手を正確に特定
- 特定できない場合（全イベント rbi=0 等）は従来通り先発にフォールバック

---

### 2026-04-21 — feat: 投手タブ画面リデザイン・7回担当指名投手追加

**仕様本文への影響あり（§4 ゲームシステム — 継投管理）**

- 投手ビューのセクション順を変更: 投手陣テーブル（削除）→ ①先発ローテーション → ②継投 → ③その他中継ぎ投手
- 「指名投手」セクションを「継投」にリネーム。スロットを 9回（抑え）・8回（セットアッパー）・7回担当の3本に拡張
- `pitchingPattern.seventhId` フィールドを追加。7回限定の指名投手を設定可能
- `simulation.js` に `'seventh'` targetRole を追加。7回ブリッジ状況（inning=7, lead -2〜+4）で `seventhId` 指名投手を優先起用
- 投手陣の全体テーブルを削除し、球速・制球・変化球・ERA・WHIP・強化フォーカス・↓二軍ボタンを各セクション行に移植

---

### 2026-04-20 — feat: 複数守備適正・コンバートシステム（e82bcc6）

**仕様本文への影響あり（§5 データモデル）**

- `player.positions: { [pos]: number }` フィールドを新設。選手生成時にポジションごとの習熟度（0〜100）を管理
- 生成時ルール: 遊撃手→二塁手・三塁手、二塁手→遊撃手・三塁手、中堅手→左右翼手、左右翼手→もう一方の外野、一塁手→左右翼手、捕手・三塁手→一塁手（各75%確率）
- `player.convertTarget: string|null` フィールド追加。コンバート指示中のポジションを保持し、試合進行ごとに+0.5習熟度上昇
- `simulation.js` の `fieldingLevel` 計算に習熟度ペナルティを適用（`0.7 + prof/100 * 0.3` 乗数）
- `RosterTab` の野手テーブルに「適正」列（副守備位置と習熟度）・「コンバート」列（目標ポジション選択）を追加

---

### 2026-04-20 — fix: 年度別成績タブの通算OPSがNaNになるバグを修正

**仕様本文への影響なし（内部バグ修正のみ）**

- 根本原因: `CareerTable.jsx` の `totals` オブジェクトに `D`（二塁打）・`T`（三塁打）・`BB`・`HBP`・`SF`・`K` が含まれておらず、`saberBatter(totals)` 内でこれらが `undefined` → `NaN` に伝搬してOPSがNaNになっていた
- 修正内容: `totals` の `sumK` 呼び出しに欠けていた6フィールドを追加。また個別年度行・通算行のOPS表示に `>0` ガードを追加し、打席なし選手で "NaN" ではなく "---" を表示するよう統一
...
- 根本原因: `useGameState.js` の `useReducer` 初期 state で `year: 2025` とハードコードされていた
- 修正内容: `year: 2026` に変更。2026年はシーズン日程を `getDefaultParams(2026)` で自動算出（開幕3/27、交流戦6/3等）
...
#### 機能A: 他チーム試合結果の閲覧（全チームボックススコア対応）

- `computeBoxScore()` を `postGame.js` に追加: `log` / `inningSummary` からイニング別得点・打撃成績・投手成績を算出するピュア関数
- `allTeamResultsMap` 状態を新設（ランタイムのみ、localStorage 非保存）: 全チームの試合ごとにボックススコアを蓄積。`handleAutoSimEnd` / `handleTacticalGameEnd` / `runBatchGames` の3パスすべてで記録
- `TeamDetailScreen.jsx` を新規作成: チーム情報のフルスクリーン表示（ロスター・成績 / 日程・結果 / 移籍履歴タブ）。`TeamModal` のオーバーレイ形式を廃止し `screen="team_detail"` で専用画面へ遷移
- `BoxScoreModal.jsx` を新規作成: イニング別スコア表 / 打撃成績表（AB・H・HR・RBI・BB・K）/ 投手成績表（IP・H・ER・BB・K・結果 W/L/S）
- 日程・結果タブで試合結果ボタンをクリックすると BoxScoreModal を表示

#### 機能B: 試合前バリデーション強化

- `buildSimLineup()` を導入: CPU チームのラインナップを自動補完（外国人選手4人枠トリム + ポジション優先順で不足分補填）
- `handleStartGame` に試合前チェック追加: 一軍登録人数 MAX_ROSTER 超過 / 打順人数不足 / 外国人打順枠超過

#### 機能C: 試合前エラーダイアログ

- `pregameError` 状態を新設: バリデーション失敗時にオーバーレイダイアログを表示
- 「ロースタータブへ」ボタン（自動遷移なし）+ 「閉じる」ボタン

---

### 2026-04-16 — feat: 采配モード 攻守インジケーター＋当日成績表示

**仕様本文への影響あり（§15.9 追加）**

- スコアボード直下に全幅の攻守バナーを追加（攻撃中: 緑「⚔️」/ 守備中: 青「🛡️」）
- バナー右端に次打者名と当日の打撃成績（打数・安打数・HR）をコンパクトに表示
- 自チーム投手カードに当日成績（対戦打者数 / 奪三振K / 被安打 / 失点）を追加
- 次打者 vs 投手カードに当日打撃成績（打数−安打・HR・打点・四球）を追加
- 代打候補リストの各選手に当日成績をインライン表示
- モメンタムバー UI を廃止（ゲームロジックは維持）
- `getGameBattingStats` / `getGamePitchingStats` ヘルパー関数を TacticalGame.jsx に追加
- CSS に `.side-banner` / `.gstat` クラスを追加
...
- `src/components/tabs/BalanceTab.jsx` を新規作成
- ハブ画面タブバーに「分析」グループを追加し「⚖️ リーグ分析」タブを配置
- **Section 1: シーズン実績** — リーグ全12チームの打者・投手成績を集計し、打率/K率/BB率/HR率/ERA を NPB 2024 基準値と比較表示（5試合以上で表示）
- **Section 2: クイックシム** — 「実行」ボタンで合成選手 800 PA × 3 パターン（平均/強/一流）をその場でシミュレーション。実際のエンジン確率を NPB 基準と即時比較可能
- **Section 3: バランス設定値** — ABILITY_RANGE の天井値・単打抑制係数を読み取って表示。パラメータ変更の効果確認に使用

---

### 2026-04-16 — balance: 投高打低緩和（シミュレーションエンジン）

**仕様本文への影響あり（§7）**

- `ABILITY_RANGE.contact.hi`: `0.240` → `0.260`（一流打者の安打率天井を引き上げ。投手との能力幅の非対称性を緩和）
- `ABILITY_RANGE.p_stuff.hi`: `0.300` → `0.265`（最上位投手の K 率天井を 30%→26.5% に引き下げ）
- `calcPAProbs` 単打抑制係数: `/ 300` → `/ 400`（投手 stuff による安打率低下を緩和。pitStuff=70 で 6.7%→5%減、pitStuff=80 で 10%→7.5%減）
