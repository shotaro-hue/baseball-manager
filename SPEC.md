...
| `morale` | number(0〜100) | 個人モラール。打撃・投球能力に `(morale-70)/250` を加算 |
...
| **個人モラール** | 各選手の `morale`（0〜100）を打者・投手能力に加算（`(morale-70)/250`。70が中立、100で+12%、40で-12%） |
...
```
ticket  = round(budget * 0.3 * (0.6 + popularity/200 + winPct*0.4) / SEASON_GAMES * 1000)
...
### 4.13 モラールシステム（Tier 3 ⑨）

#### シーズン末モラール変動

各選手のモラールはシーズン終了時に以下の要素で変動する。
...
- **⑨ 選手モラール**（§4.3・§4.4・§4.13）
  ...
  - `applyBatterSituation` / `applyFatigue` で個人モラールを能力に反映
