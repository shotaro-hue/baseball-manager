...
> 最終更新: 2026-04-29（仕様書運用ルール・参照整合性を更新）
...
### シミュレーション確率レンジ（`ABILITY_RANGE` in `simulation.js`）

| パラメータ | lo | mid（ベースライン） | hi | 説明 |
|---|---|---|---|---|
| `contact` | 0.100 | 0.1656 | **0.240** | 打者の単打確率レンジ（2026-04-26 に 0.260 から引き下げ） |
| `power` | 0.003 | 0.0175 | 0.050 | 打者の本塁打確率レンジ |
| `eye` | 0.030 | 0.0727 | 0.130 | 打者の四球確率レンジ |
| `p_stuff` | 0.120 | 0.1990 | **0.265** | 投手の三振確率レンジ（2026-04-16 に 0.300 から引き下げ） |
| `p_control` | 0.130 | 0.0727 | 0.030 | 投手の四球確率レンジ（逆順: 制球が良いほど低値） |
| `p_power` | 0.030 | 0.0175 | 0.005 | 投手速球による本塁打抑制レンジ（逆順） |

**単打抑制係数**（`calcPAProbs` 内）: `1 - (pitStuff - 50) / 400`（2026-04-16 に `/300` から緩和）

### リーグ環境係数（`DEFAULT_LEAGUE_ENV` in `simulation.js`）

ユーザーがバランスタブの「リーグ環境設定」スライダーで上書き可能。値は `myTeam.leagueEnv` に保存される。

| パラメータ | デフォルト値 | 説明 |
|---|---|---|
| `kMod` | **0.89** | 三振率乗数。<1 で投手K率を下げる（打者有利）（2026-04-25 に 1.00 から変更） |
| `hitMod` | **1.02** | 安打率乗数。>1 で安打が増える（打者有利）（2026-04-26 に 1.10 から引き下げ、打率過剰抑制） |
| `hrMod` | **1.35** | 本塁打率乗数。>1 でHRが増える（2026-04-26 に 1.00 から変更、HR率 1.3%→1.8% 目標） |
| `bbMod` | **1.30** | 四球率乗数。>1 で四球が増える（2026-04-26 に 1.00 から変更、四球率 5.6%→7.3% 目標） |
...
### 15.9 采配モード 攻守インジケーター＋当日成績表示 ✅ 実装済み（2026-04-16）

`src/components/TacticalGame.jsx` の試合操作 UI を改善。

#### 変更内容

| 変更 | 詳細 |
|---|---|
| **攻守バナー** | スコアボード直下に全幅バナーを追加。自チーム攻撃中は緑テーマ「⚔️ 攻撃中」、守備中は青テーマ「🛡️ 守備中」でイニング・相手チーム名・次打者名＋当日成績を表示 |
| **投手カード当日成績** | 左パネルの自チーム投手カードに「今日: N打者 NK N被安打 N失点」を追加 |
| **次打者カード当日成績** | 次打者 vs 投手カードに「今日: N打N安打 NHR N打点 N四球」を追加 |
| **代打メニュー当日成績** | 代打候補リストの各選手に「N打N安」のインライン表示を追加 |
| **モメンタムバー削除** | モメンタムバー UI を廃止（ゲームロジックは維持） |

#### 実装詳細

`getGameBattingStats(log, playerId)` / `getGamePitchingStats(log, pitcherId)` をコンポーネント外ヘルパーとして追加し、`gs.log` を batId / pitcherId でフィルタしてその場で集計。CSS に `.side-banner` / `.gstat` クラスを追加。

### 15.8 試合結果画面リデザイン ✅ 実装済み（2026-04-15）

`src/components/Screens.jsx` の `ResultScreen` を Yahoo Baseball スタイルに再設計。

#### 画面構成（上から）

| セクション | 内容 | データソース |
|---|---|---|
| スコアヘッダー | チーム絵文字・スコア・VICTORY/DEFEAT バナー・通算成績 | `gsResult.score`・`myTeam`・`oppTeam` |
| イニング別スコア | 1〜9回（延長対応）＋計＋安列 | `gsResult.inningSummary` |
| 責任投手 | 勝/敗/S（両チーム分） | `gsResult.log` から計算 |
| 本塁打 | イニング順・ソロ/2ラン/3ラン/満塁 | `gsResult.log` から抽出 |
| 打者成績タブ | 自チーム＋相手チーム：打率・打・安・点・本 | `gsResult.log` + `myTeam.players.stats` |
| 投手成績タブ | 自チーム＋相手チーム：回・安・振・四・失＋勝/敗/S | `gsResult.log` から計算 |

#### 責任投手判定ロジック（UI層のみ）

- 勝利投手（自チーム）: 先発がアウト≥15かつ勝利 → 先発、それ以外 → 最初の中継ぎ
- 敗戦投手: 自チーム/相手先発（簡易版）
- セーブ: 勝利かつ最終差1〜3点かつ最後の投手≠勝利投手の場合
